<script lang="ts">
    ...
    let entryTypes = [
        'article', 'book', 'web', 'conference', 'report', 'thesis', 'misc', 'reference', 'periodical', 'proceedings'
    ];
...
<div class="navbar bg-base-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-2 px-4">
                    ...
                    <table class="table table-zebra w-full">
                        ...
                        <tbody>
                            {#each Object.entries(entries) as [id, entry]}
                                ...
                                >
                                    <td class="font-semibold truncate max-w-xs">
                                        {#if entry.title}
                                            {entry.title}
                                        {:else}
                                    ...
                                    <td class="truncate max-w-xs">{Array.isArray(entry.author) ? entry.author.join('; ') : entry.author}</td>
                                    <td>{entry.type}</td>
                                    <td>{entry.date ? (typeof entry.date === 'string' ? entry.date.slice(0, 4) : entry.date) : ''}</td>
                                    <td>
                                        <button
